...
/*[4] Parallax
$(parallax_box).parallax({imageSrc: 'img/home-background.jpeg'});
*/
/*[5] Images Loaded(Shuffle Filter)
...
$(window).on("scroll", function () {
  ...
  if ($this.scrollTop() >= 100) {
    if ($this.innerWidth() > 991) {
      nav_link.addClass("small-nav-edited");
    }
    if ($this.innerWidth() <= 576) {
      top_bar.hide();
    }else {
      top_bar.css("height", "35");
    }
  }
  if ($this.scrollTop() < 100) {
    if ($this.innerWidth() > 991) {
    ...
    if ($this.innerWidth() <= 576) {
      top_bar.show();
      top_bar.css("height", "65");
    } else {
      top_bar.css("height", "44");
    }
  }
});
/* Scroll to top */
